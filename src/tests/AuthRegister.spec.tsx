...
describe("Register component", () => {
  ...
  it("should handle form submission with valid input", async () => {
    ...
    require("react-firebase-hooks/auth").useAuthState.mockReturnValue([
      mockUser,
      mockLoading,
    ]);
  ...
  it("should show validation errors for invalid input", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(messages["Register"]["name-error"]),
      ).toBeInTheDocument();
      expect(
        screen.getByText(messages["Register"]["email-error"]),
      ).toBeInTheDocument();
      expect(
        screen.getByText(messages["Register"]["password-error"]),
      ).toBeInTheDocument();
    });
  });
  ...
  it("should navigate to the user`s locale when user is authenticated", async () => {
    ...
    expect((useAuthState as jest.Mock).mock.results[0].value[0]).toEqual(
      mockUser,
    );
  });
});
